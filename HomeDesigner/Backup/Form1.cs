  ...
  public class Form1 : Form
  {
    ...
    public void refreshSimulazione()
    {
     ...
     String t0= intervallo.Split('-')[0];
      ...
      pauseSimulazione = false;
      Task task = Task.Run( () => this.pictureBox1.Image = (Image)this.data.drawSimulate(this, folderPath,
          this.pictureBox1, this.placepcb.Checked, this.sensorpcb.Checked,
    ...
    public void refreshPrediction( Dictionary<string, List<KeyValuePair<Position,Color>>> dictionary, DateTime timeInizio, DateTime timeFine )
    {
      ...
      stopPredizione = false;
         Task task = Task.Run( () => this.pictureBox1.Image = (Image) this.data.drawPrediction( this.pictureBox1,dictionary,this.placepcb.Checked, this.sensorpcb.Checked, this.wallpcb.Checked, timeInizio, timeFine));
    ...
    private void buttonSimula_Click(object sender, EventArgs e)
    {
      ...
      if (formSimulation.Utenti.Count == 0 )
      {
        return;
         ...
         this.data.setPath();
      Task task = Task.Run( () =>    this.data.simulationHandler(formSimulation.Utenti,formSimulation.TipoTraietoria));      
      if (task.IsCompleted)
      {
        task.Dispose();
        task = null;
